...
DECLARE
  ...
  -- tbl1
  tbl1_select_query text 		= '(select tbl1.item, tbl1.TransCode as trcode, ';
  tbl1_select_base text    		= 'from(select DISTINCT(a.description) as item, a.code || ''0000000'' as TransCode, ';
  ...
  tbl1_where_statement text     = 'WHERE a.is_deleted = false AND a.fk_transaction_classification = '||transaction_classification||' '; 
  tbl1_closing_statement text   = 'group by a.description, a.code || ''0000000'', b.curcde1, z.fk_currency)tbl1 group by tbl1.item, tbl1.TransCode ORDER BY RIGHT(LEFT(tbl1.TransCode,3),1), tbl1.TransCode)tbl1 JOIN ';  
  
  -- tbl2
  tbl2_select_query text   		= '(select tbl1.item, tbl1.TransCode as trcode, to_char(SUM(tbl1.otherCurrencies), ''FM999999990.00'') as otherCurrenciesInUSD from(';
  tbl2_select_base text 		= 'select DISTINCT(a.description) as item, a.code || ''0000000'' as TransCode, sum(coalesce(b.amtorig1, 0.00) * coalesce(rates.usd_rate, 0.00)) as otherCurrencies ';
  ...
  tbl2_where_statement text     = 'WHERE a.is_deleted = false AND a.fk_transaction_classification = '||transaction_classification||' AND z.id IS NULL ';  
  tbl2_closing_statement text   = 'group by a.description, a.code || ''0000000'', b.curcde1, z.fk_currency, rates.usd_rate)tbl1 group by tbl1.item, tbl1.TransCode ORDER BY RIGHT(LEFT(tbl1.TransCode,3),1), tbl1.TransCode)tbl2 ON tbl2.trcode = tbl1.trcode';  
